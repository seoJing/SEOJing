...
<Paragraph>
  이 글의 목표는 한 가지입니다. <strong>OpenClaw 위에서 일어나는 모든 일을
  같은 평면 위에 얹어 볼 수 있는 좌표계</strong>를 머리에 만드는 것. 좌표계가
  잡혀 있으면 다음 글들은 그 위에 점을 찍는 작업이 되고, 좌표가 없으면 글마다
  각기 다른 평면 위에서 헤매게 됩니다.
</Paragraph>
...
<Paragraph>
  ...
  에이전트가 같은 도구·스킬·메모리·외부 채널을 공유하면서 일할 수 있게 묶어
  주는 로컬 런타임입니다. 운영은 <code>openclaw</code> CLI로 하고, 코어 역할을
  하는 프로세스 한 개(<strong>gateway</strong>)가 거의 모든 것을 잡고 있습니다.
</Paragraph>
...
<Paragraph>
  처음 한 번만 머리에 박아두시면 다른 모든 글이 이 좌표 위에 얹혀서 읽힙니다.
  세 줄로 요약하면 이렇습니다. <strong>외부에서 들어온 메시지가 어떤 에이전트의
  어떤 세션으로 가서, 그 세션이 어떤 도구·스킬을 쓰고, 결과를 어떤 메모리에
  남기느냐.</strong>
</Paragraph>
...
<Paragraph>
  표가 다소 빽빽해 보일 수 있지만, 정말 외워야 하는 것은 위쪽의 5개{" "}
  (Channel · Agent · Session · Tools · Skills · Memory) 흐름입니다. Gateway와
  Workspace는 그것들이 사는 "장소"라고 보시면 됩니다.
</Paragraph>
...
<Subtitle level={3}>4-1. <code>~/.openclaw/</code> — 게이트웨이 운영 데이터</Subtitle>
...
<Subtitle level={3}>4-2. <code>~/.openclaw/workspace/</code> — 자아·작업 공간</Subtitle>

<Paragraph>
  여기가 진규의 "OS 위에 깔린 홈디렉토리"에 해당합니다. 자아 파일들과 일별
  메모리, 프로젝트 클론, 운영 노트, 워크스페이스 스킬이 모두 여기에 삽니다.
  세션 시작 시 자동으로 읽히는 파일도 다 이쪽에 있습니다.
</Paragraph>
...
<Paragraph>
  ...
  <code>~/.local/share/fnm/.../node_modules/openclaw/</code>에 있습니다.
  이 안에서 자주 들여다보게 되는 폴더는 셋입니다.{" "}
  <strong>docs/</strong>(공식 문서), <strong>skills/</strong>(시스템 스킬),
...
<Paragraph>
  ...
  <strong>"누가 push 버튼을 누르는가"</strong>입니다. 코딩 위임을 Hermes에
  넘긴 케이스를 예로 들면, Hermes가 직접 push 버튼을 누르게 둘 수도 있고,
  오케이징이 결과를 받아서 검토 후 push할 수도 있습니다. 진규 환경은 최근에
  후자로 바꿨습니다 — 검토 단계가 빠지면 컨텍스트가 즉시 증발하기 때문입니다.
  이 결정은 Layer 4(위임)에서 진하게 다룹니다.
</Paragraph>
...
<Paragraph>
  지금은 머신이 닫혀 있고 Tailscale도 꺼져 있어서 위험은 0에 가깝습니다.
  모바일 노드를 본격적으로 페어링하기 시작하면 이 표가 다시 살아납니다.
  Layer 6에서 이어서 다룹니다.
</Paragraph>
...
<Paragraph>다음 글을 시작하기 전에 직접 해보면 좋은 정찰이 셋 있습니다.</Paragraph>
...
<Paragraph>
  Layer 1에서는 이 좌표 위의 가장 무거운 한 점인 <strong>gateway</strong>를
  직접 분해합니다. config schema, hot-reload 가능한 부분과 restart가 필요한
  부분, 세션 모델, 도구 정책 — 다음 글에서 함께 보겠습니다.
</Paragraph>
