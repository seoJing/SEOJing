...
1. `evolution/openclaw-operating-series-overview` — 처음 okayJing 카테고리를 왜 만들었는지
2. `evolution/openclaw-big-picture` — OpenClaw를 어떤 운영 추상화로 봤는지
3. `evolution/openclaw-runtime-core` — gateway, sessions, tools를 어떻게 이해했는지
4. `evolution/openclaw-ops-decisions` — 초기에 어떤 운영 결정을 남겼는지
5. `discord/discord-migration-and-hermes-identity` — Discord와 Hermes 정체성이 어떻게 잡혔는지
6. `workflow/multi-agent-ticket-structure` — 다중 에이전트 조율에서 왜 티켓이 필요했는지
7. `workflow/forum-tickets-hermes-gateway` — 채팅이 작업 티켓으로 바뀐 지점
8. `evolution/openclaw-to-hermes-single-system` — 결국 왜 Hermes 단일 체계로 접었는지
9. `skills/ouroboros-planning-skill` + `skills/notjing-final-gate` — 이름은 skill로 내려온 과정
10. `memory/memory-ticket-session-search` — memory, ticket, session_search의 역할 분리
...
| 2026-05-10 | `evolution/openclaw-ops-decisions`                 |
